...
"""Carla client for AV rental application.

   First run carla server

   CarlaUE4.exe -quality-level=Low -DX11
...
class World(object):
    ...
    def __init__(self, carla_world, hud, datawriter, args):
        """Constructor method"""
        self._args = args
        ...
        # Set recording to on by default to output camera images to local
    ...
    def restart(self, args):
        ...
        # Get user selected brand and model
        ...
        while self.player is None:
            ...
            try:
                ...
                print("Failed to spawn the actor %s" % err)
            self.modify_vehicle_physics(self.player)
...
class HUD(object):
    ...
    def __init__(self, width, height, datawriter):
        """Constructor method"""
        ...
        self.connection_json_data = {'connection_status': 'connected'}
        # Trip ID to be written to local file
        self.trip_id = None
    ...
    def tick(self, world, clock):
        ...
        vehicles = [(self.dist(x.get_location(), transform), x) for x in vehicles if x.id != world.player.id]
...
class CollisionSensor(object):
    ...
    def __init__(self, parent_actor, hud, datawriter):
        """Constructor method"""
        self.sensor = None
        self.history = []
...
def main():
    ...
    # Car selection in restart() was changed from filter() to find() a specified vehicle
    # The default argument will no longer work.
    argparser.add_argument(
        '--filter',
        metavar='PATTERN',
        default='vehicle.*',
    ...
    argparser.add_argument(
        '--write_to_db',
        help='Writes data to the DB (default: False)',
        default=False,
        type=bool)
    argparser.add_argument(
        '--recording',
        help='Records the camera images (default: False)',
        default=False,
        type=bool)
    argparser.add_argument(
    ...
    argparser.add_argument(
        '--source',
        help='city string which is converted to simulator starting location')
    # Add dropoff location argument, named it dest
    argparser.add_argument(
        '--dest',
