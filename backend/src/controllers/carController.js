...
const getRepairCars = (req, res) => {
  let sql = "Select * from Cars where Cars.ownerId=? and status=?";
  conn.query(sql, [req.user.email, "Repair"], (err, result) => {
    if (err) {
      console.log(err);
      return;
    }
...
const makeActive = (req, res) => {
  console.log(req.body);
  let sql = "update Cars set status=? where id=?";

  conn.query(sql, ["Active", req.body.id], (err, result) => {
    if (err) {
      console.log(err);
      return;
    }
...
const getOwnerCarRides = (req, res) => {
  let carIdList = [];
  let carIdModel = [];
  let sql = "Select id,make from Cars where Cars.ownerId=? ";
  conn.query(sql, [req.user.email], (err, result) => {
    if (err) {
      console.log(err);
      return;
